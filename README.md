...
9. [L148](https://leetcode.com/problems/sort-list/): Given the `head` of a linked list, return the list after sorting it in ascending order.
  ...
  * In the bottom-up solution, space complexity is `O(1)`. [Bottom-up Solution in Python](https://leetcode.com/submissions/detail/638306894/) or [Bottom-up Solution in Java (Discussion)](https://leetcode.com/problems/sort-list/discuss/46712/Bottom-to-up(not-recurring)-with-o(1)-space-complextity-and-o(nlgn)-time-complextity). And for this solution, I'd suggest understanding the idea and remembering it since you are not likely to come up with an elegant, bug-free answer for the follow-up question (i.e. you can only use constant exxtra space).
